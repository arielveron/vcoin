...
export default function AchievementSection({
  ...
  return (
    <div className="w-full">
      {/* Achievement Summary Card - Hide if hideHeader is true */}
      {!hideHeader && (
        <div
          className={`bg-gradient-to-r from-blue-50 to-purple-50 border border-blue-200 rounded-lg p-4 transition-all duration-200 ${
            !forceExpanded ? 'cursor-pointer hover:shadow-md' : ''
          }`}
      ...
      {isExpanded && (
        <motion.div
          initial={forceExpanded ? { opacity: 1, height: "auto" } : { opacity: 0, height: 0 }}
          animate={{ opacity: 1, height: "auto" }}
          exit={{ opacity: 0, height: 0 }}
          transition={{ duration: forceExpanded ? 0 : 0.3, ease: "easeInOut" }}
