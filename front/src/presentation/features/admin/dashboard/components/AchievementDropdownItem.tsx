...
export default function AchievementDropdownItem({
  ...
  return (
    <div className={`flex gap-2 items-center space-x-2 sm:space-x-3 hover:bg-gray-300 bg-gray-200 mb-1 rounded ${config.container}`}>
      <div
        className={`${config.iconContainer} rounded-full flex items-center justify-center ${rarityStyle.bgColor} flex-shrink-0`}
      >
        <IconRenderer
      ...
      <div className="flex-1 min-w-0">
        <div className={`${config.title} font-bold text-gray-900 truncate text-left`}>{achievement.name}</div>
        {showFullDescription && (
          <div className={`${config.description} text-gray-500 truncate text-left`}>{achievement.description}</div>
        )}
        <div className="flex items-center space-x-2 sm:space-x-3 mt-1">
          <span className={`${config.rarity} rounded ${rarityStyle.bgColor} ${rarityStyle.textColor}`}>
            {achievement.rarity}
          </span>
          <span className={`${config.points} text-purple-600 font-medium`}>{achievement.points} pts</span>
        </div>
      </div>
    </div>
  );
